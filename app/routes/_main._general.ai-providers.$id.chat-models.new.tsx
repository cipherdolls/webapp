...
import type { Route } from './+types/_main._general.ai-providers.$id.chat-models.new';
...
export default function NewChatModel({ params }: Route.ComponentProps) {
  const { data: aiProvider } = useAiProvider(params.id!);
  const { mutate: createChatModel, error: createChatModelError } = useCreateChatModel();
  ...
  return (
    ...
    >
      <Modal.Content>
        <Modal.Title>Add Chat Model for {aiProvider?.name}</Modal.Title>
        <Modal.Description className='sr-only'>Add Chat Model for {aiProvider?.name}</Modal.Description>
