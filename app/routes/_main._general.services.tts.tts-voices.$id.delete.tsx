...
export function meta({}: Route.MetaArgs) {
  return [{ title: 'Delete TTS Voice' }];
}

export default function TtsVoiceDelete() {
  const params = useParams();
  const [searchParams] = useSearchParams();
  const ttsVoiceId = params.id;
  const voiceName = searchParams.get('name') || 'Voice';
  ...
  return (
    ...
    >
      <Modal.Content>
        <Modal.Title className='sr-only'>Delete TTS Voice</Modal.Title>
        <Modal.Description className='sr-only'>Delete TTS Voice</Modal.Description>
        <fetcher.Form
          action={`/tts-voices/${ttsVoiceId}/destroy`}
          method='DELETE'
        ...
        >
          <Modal.Body className='flex flex-col gap-3.5'>
            ...
            <div className='flex flex-col gap-2'>
              <h2 className='text-heading-h2 font-semibold text-center text-base-black'>Delete {voiceName}?</h2>
