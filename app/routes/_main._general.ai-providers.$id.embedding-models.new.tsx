...
import type { Route } from './+types/_main._general.ai-providers.$id.embedding-models.new';
...
export default function NewEmbeddingModel({ params }: Route.ComponentProps) {
  const { data: aiProvider } = useAiProvider(params.id!);
  const { mutate: createEmbeddingModel, error: createEmbeddingModelError } = useCreateEmbeddingModel();
