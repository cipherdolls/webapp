...
const YourChats = ({ chats, avatars }: YourChatsProps) => {
  ...
  const leftChatGroup = groupedChats.slice(0, 4)
  const rightChatGroup = groupedChats.slice(4)
            ...
            <div className='flex flex-col gap-2'>
              {leftChatGroup.map((group, index) => (
                <div className={`${!showAll && index >= 2 ? 'hidden' : 'transition-all duration-500 ease-out'}`} key={index}>
                  <div
                    className={cn('flex items-center gap-3 select-none cursor-pointer bg-base-white hover:bg-white/80 rounded-xl', showAll && index >= 2 && 'animate-show-all')}
