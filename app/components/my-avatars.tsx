...
const MyAvatars = ({ avatars }: { avatars: Avatar[] }) => {
  ...
  const displayedAvatars = showAll ? sortedAvatars : sortedAvatars.slice(0, 4);
  ...
  return (
    <div className='flex flex-col gap-5'>
      <h3 className='text-heading-h3 text-base-black'>My Avatars</h3>
      <div className={`grid md:gap-5 gap-3.5 ${displayedAvatars.length === 1 ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2'}`}>
        {displayedAvatars.map((avatar, index) => (
          <div
            className='p-4 shadow-bottom-level-1 bg-white rounded-xl flex lg:items-center lg:flex-row flex-col justify-between gap-4'
            key={index}
          >
