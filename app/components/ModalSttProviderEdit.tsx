...
import { useUpdateSttProvider } from '~/hooks/queries/sttMutations';
import type { SttProvider } from '~/types';

export default function ModalSttProviderEdit({ sttProvider, onClose }: { sttProvider: SttProvider | null; onClose?: () => void }) {
  const { mutate: updateSttProvider, isPending: isUpdatingSttProvider, error: updateSttProviderError } = useUpdateSttProvider();
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    updateSttProvider(
      { sttProviderId: sttProvider!.id, formData },
      {
        onSuccess: () => {
          handleClose();
        },
  ...
  return (
    <Modal.Root open={Boolean(sttProvider)} onOpenChange={(open) => !open && onClose?.()}>
      <Modal.Content>
        {sttProvider && (
          <>
            ...
            <form className='w-full flex flex-col mt-[18px]' onSubmit={handleSubmit}>
              <Modal.Body className='flex flex-col gap-5'>
                <ErrorsBox errors={updateSttProviderError} />
                    ...
                    <div className='absolute z-10 bottom-0 translate-y-1/2 left-1/2 -translate-x-1/2'>
                      <div className='flex items-center justify-between w-full'>
                        ...
                        >
                          ...
                          {(selectedImage && sttProvider.picture) &&
