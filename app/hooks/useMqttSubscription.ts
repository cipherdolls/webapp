...
export const useMqttSubscription = (
  ...
  useEffect(() => {
    ...
    if (unsubscribeRef.current) {
      console.log(`[useMqttSubscription] Cleaning up existing subscription for topic: ${topic}`);
    ...
    if (!enabled || !topic || !connectionState.isConnected) {
      console.log(`[useMqttSubscription] Skipping subscription - enabled: ${enabled}, topic: ${topic}, connected: ${connectionState.isConnected}`);
      return;
    }

    // Establish new subscription
    console.log(`[useMqttSubscription] Setting up subscription for topic: ${topic}`);
    unsubscribeRef.current = subscribe(topic, stableCallback);

    // Cleanup function
    return () => {
      if (unsubscribeRef.current) {
        console.log(`[useMqttSubscription] Cleaning up subscription for topic: ${topic}`);
        unsubscribeRef.current();
        unsubscribeRef.current = null;
      }
    };
